...
export function createSimulation(opts) {
    ...
    function applyAction(action) {
        ...
        switch (action.type) {
            ...
            case "read": {
                ...
                const pageText = BookCore.generateBookPage(gs.side, gs.position, gs.floor, bi, 0, seed);
                const sensibility = BookCore.scoreSensibility(pageText);
                const dwellResult = BookCore.dwellMoraleDelta(sensibility, gs.nonsensePagesRead);
    ...
    function onDawn() {
        // Resurrection
        if (gs.dead) {
            gs.stats = Surv.applyResurrection();
            gs.dead = false;
            gs.deathCause = null;
            gs.deaths++;
            gs.despairing = false;
