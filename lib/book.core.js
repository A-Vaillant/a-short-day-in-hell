...
/** Max characters per generated page (truncated at word boundary). */
export const PAGE_CHAR_LIMIT = 950;
...
export function generateBookPage(side, position, floor, bookIndex, pageIndex, globalSeed, stories, dictionary) {
    if (!stories || stories.length === 0 || !dictionary || dictionary.length === 0) {
        return { text: "", storyId: -1, editDistance: 0 };
    }
    ...
    let text = detokenize(degraded);
    if (text.length > PAGE_CHAR_LIMIT) {
        // Truncate at last word boundary
        const cut = text.lastIndexOf(" ", PAGE_CHAR_LIMIT);
        text = cut > 0 ? text.slice(0, cut) : text.slice(0, PAGE_CHAR_LIMIT);
    }
